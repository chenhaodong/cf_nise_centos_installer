...
for process in \
    ...
    nats
do
    sudo ${NISE_PATH}/bosh/bin/monit start $process
    sleep 30
done;

sudo ${NISE_PATH}/bosh/bin/monit start all


echo "Waiting for all processes to start"
for ((i=0; i < 120; i++)); do
    if ! (sudo ${NISE_PATH}/bosh/bin/monit summary | tail -n +3 | grep -v -E "running$"); then
        break
    fi
    sleep 10
done
